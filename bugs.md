...
```
  # Edit: /etc/nginx/sites-available/meetapp.pl

  location /event/ {
      proxy_pass http://localhost:8080/api/v1/web/event/;
  ...
  location /group/ {
      proxy_pass http://localhost:8080/api/v1/web/group/;
      proxy_set_header Host $host;
      proxy_set_header X-Real-IP $remote_addr;
      proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
      proxy_set_header X-Forwarded-Proto $scheme;
...
- walidacja podczas rejestrowania konta chyba nie ma prawidlowych tlumaczen
...
- walidacja hasła przy rejestracji
  - tlumaczenie komunikatu z serwera
  - tlumaczenie dla rate limiting
  - zly komunikat o wymaganych znakach
